...
int main(int argc, char *argv[]) {
	...
	for (size_t m = 0; m < num_iters; ++m) {
		...
		for (auto i = 4UL; i <= num_iters * vec_inc * vec_width; i += vec_inc * vec_width) {
			...
			const auto fi2 = _mm256_cvtepi32_pd(_mm_set1_epi32(i + vec_width));
			const auto t2 = _mm256_add_pd(fi2, fi_offset);
			const auto indices2 = _mm256_mul_pd(_mm256_set1_pd(h), t2);
			const auto y2 = f(indices2);

			sum = _mm256_add_pd(sum, _mm256_add_pd(y, y2));
		}
